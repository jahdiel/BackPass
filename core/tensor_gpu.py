...
class TensorGPU:
    ...
    def pass_grads(self):
        # Calculate VJP
        parent_vals = [p.value if isinstance(p, TensorGPU) else p for p in self.parents]
    ...
    def tensor_parents(self):
        return [parent for parent in self.parents if isinstance(parent, TensorGPU)]
    ...
    def __getitem__(self, key):
        ...
        raise TypeError("'TensorGPU' object is not subscriptable")

    def __repr__(self):
        if self.parents == None:
            return '<backpass.core.tensor_gpu.TensorGPU {} -leaf ref: {}>'.format(self.value, self.ref)
        return '<backpass.core.tensor_gpu.TensorGPU {} ref: {}>'.format(self.value, self.ref)

    def __neg__(self):
        import backpass.cupy as primitives
